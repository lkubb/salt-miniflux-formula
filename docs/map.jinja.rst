...
We will see a quick setup to configure the ``miniflux`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``miniflux`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``miniflux`` formulas under ``/srv/salt/miniflux/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/miniflux/parameters/dns:domain/
    ln -s dns:domain /srv/salt/miniflux/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/miniflux/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/miniflux/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``miniflux`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``miniflux`` formulas under ``/srv/salt/miniflux/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/miniflux/parameters/roles
...
We create a configuration for the role ``miniflux/server`` in ``/srv/salt/miniflux/parameters/roles/miniflux/server.yaml``:
...
We create another configuration for the role ``miniflux/client`` in ``/srv/salt/miniflux/parameters/roles/miniflux/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles miniflux/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles miniflux/client
...
.. code-block:: console
    ...
    mv /srv/salt/miniflux/parameters/dns:domain/  '/srv/salt/miniflux/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-miniflux-as-json:
      test.nop:
        - name: {{ miniflux | json }}
...
Here is an example based on `template-formula/miniflux/config/file.sls`_:
    ...
    miniflux-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='miniflux-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ miniflux.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``miniflux`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ miniflux['winner'] }}
...
.. _template-formula/miniflux/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/miniflux/config/file.sls
